...
- `packages/duck/` — a minimal Discord bot that forwards requests to OpenRouter with no tools and no memory files; it reuses the shared SQLite conversation history to remember each channel's conversation across restarts
- `packages/shared/` (`@troy/shared`) — code shared between the two bots (e.g. `splitMessage`, the model constant, and the conversation-history layer: the `Conversation` entity, the SQLite store, and history helpers)
...
- `index.ts` — minimal Discord bot entry point; sends each prompt (plus the channel's recent history from the shared SQLite store) to OpenRouter and replies with the result
